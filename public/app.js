...
function precisionInsightMarkup(item) {
  const precision=item?.precision;
  if(!precision)return "";
  const regime=escapeHtml(precision.marketRegime||"UNKNOWN");
  const adx=Number.isFinite(Number(precision.indicators?.adx))?formatPrice(precision.indicators.adx):"—";
  const rank=Number.isFinite(Number(precision.relativeStrengthRank))?String(precision.relativeStrengthRank):"—";
  return `<span>Precision rejimi: ${regime} · breadth ${formatPrice(precision.breadth)}</span><span>Precision ADX: ${adx} · göreceli güç sırası ${rank} · PUANI ETKİLEMEZ</span>`;
}
...
function clientRiskBasedQuantity({capital, maxPositionPercent, entry, stop, fractional=false}) {
  const values=[capital,maxPositionPercent,entry,stop].map(Number);
  if(!values.every(Number.isFinite)||values.some(value=>value<=0)||Number(stop)>=Number(entry))return 0;
  const allocationQuantity=Number(capital)*Number(maxPositionPercent)/100/Number(entry);
  const riskQuantity=Number(capital)*0.01/(Number(entry)-Number(stop));
  const quantity=Math.min(allocationQuantity,riskQuantity);
  if(!Number.isFinite(quantity)||quantity<=0)return 0;
  return fractional?Math.floor(quantity*1e8)/1e8:Math.floor(quantity);
}
...
function renderNasdaqBrokerControls(paper) {
  ...
  mount.querySelectorAll("[data-nasdaq-broker-cancel]").forEach(button => button.addEventListener("click", async () => {
    ...
    try { renderNasdaqPaperState(await nasdaqRequest("/api/nasdaq/broker-entry/cancel", {positionId:button.dataset.nasdaqBrokerCancel})); } catch (error) { window.alert(error.message); }
  }, {once:true}));
  mount.querySelectorAll("[data-nasdaq-protection-enable]").forEach(button => button.addEventListener("click", async () => {
    try { renderNasdaqPaperState(await nasdaqRequest("/api/nasdaq/protection/enable", {positionId:button.dataset.nasdaqProtectionEnable})); } catch (error) { window.alert(error.message); }
  }, {once:true}));
}
...
async function queueCryptoPaperDecision(item) {
  ...
  const quantity = clientRiskBasedQuantity({capital:Number(latestCryptoPaperState?.initialCapital || 10000),maxPositionPercent:Number(latestCryptoPaperState?.risk?.maxPositionPercent || 20),entry:price,stop:plan.stopLoss,fractional:true});
  if (quantity <= 0) return window.alert("Risk sınırları bu plan için pozisyon açılmasına izin vermiyor.");
  try {
